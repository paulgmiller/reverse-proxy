        ...
        public async Task<IDictionary<string, Backend>> GetBackendsAsync(IConfigErrorReporter errorReporter, CancellationToken cancellation)
        {
            ...
            foreach (var pair in backends)
            {
                try
                {
                    ...
                    configuredBackends[pair.Key] = pair.Value;
                }
                catch (Exception ex)
                {
                    errorReporter.ReportError(ConfigErrors.ConfigBuilderBackendException, pair.Key, "An exception was thrown from the configuration callbacks.", ex);
                }
            }
